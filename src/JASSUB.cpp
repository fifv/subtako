...
#define CLAMP_UINT8(value)                                                                         \
    ((value > MIN_UINT8_CAST) ? ((value < MAX_UINT8_CAST) ? (int)(value * 255) : 255) : 0)
...
class BoundingBox {
  public:
    ...
    bool empty() const { return min_x == -1; }
    ...
    bool intersets(const BoundingBox &other) const {
        return !(
            other.min_x > max_x || other.max_x < min_x || other.min_y > max_y || other.max_y < min_y
        );
    }
    ...
    void clear() { min_x = max_x = min_y = max_y = -1; }
};
...
#define check_complex_tag(tag)                                                                     \
    (check_simple_tag(tag) && (begin[sizeof(tag) - 1] == '(' || begin[sizeof(tag) - 1] == ' ' ||   \
                               begin[sizeof(tag) - 1] == '\t'))
    switch (begin[0]) {
    case 'k': //-fallthrough
    case 'K':
    ...
    case 't':
        ...
        // Since all valid simple t-tags require the existence of a complex t-tag, we only check for
        // complex tags to avoid false positives from invalid simple t-tags. This makes
        // animation-dropping somewhat incorrect but as animation detection remains accurate, we
        // consider this to be "good enough"
...
class JASSUB {
  ...
  public:
    ...
    void decodeBitmap(double alpha, size_t *data, ASS_Image *img, int w, int h) {
        uint32_t color = ((img->color << 8) & 0xff0000) | ((img->color >> 8) & 0xff00) |
                         ((img->color >> 24) & 0xff);
    ...
    int allocEvent() { return ass_alloc_event(track); }

    void removeEvent(int eid) { ass_free_event(track, eid); }

    int getStyleCount() const { return track->n_styles; }

    int allocStyle() { return ass_alloc_style(track); }

    void removeStyle(int sid) { ass_free_event(track, sid); }

    void removeAllEvents() { ass_flush_events(track); }

    void setMemoryLimits(int glyph_limit, int bitmap_cache_limit) {
        printf(
            "JASSUB: setting total libass memory limits to: glyph=%d MiB, bitmap cache=%d MiB\n",
    ...
    RenderResult *renderBlendPart(const BoundingBox &rect, ASS_Image *img) {
        ...
        for (ASS_Image *cur = img; cur != NULL; cur = cur->next) {
            ...
            if (curw == 0 || curh == 0 || cur->dst_x + curw - 1 > rect.max_x ||
                cur->dst_y + curh - 1 > rect.max_y)
            ...
            for (int y = 0, bitmap_offset = 0; y < curh;
                 y++, bitmap_offset += curs, buf_line_coord += width) {
...
static uint32_t getDuration(const ASS_Event &evt) { return (uint32_t)evt.Duration; }
...
static void setEventName(ASS_Event &evt, const std::string &str) { evt.Name = copyString(str); }

static std::string getText(const ASS_Event &evt) { return evt.Text; }

static void setText(ASS_Event &evt, const std::string &str) { evt.Text = copyString(str); }

static std::string getEffect(const ASS_Event &evt) { return evt.Effect; }

static void setEffect(ASS_Event &evt, const std::string &str) { evt.Effect = copyString(str); }

static std::string getStyleName(const ASS_Style &style) { return style.Name; }

static void setStyleName(ASS_Style &style, const std::string &str) { style.Name = copyString(str); }

static std::string getFontName(const ASS_Style &style) { return style.FontName; }

static void setFontName(ASS_Style &style, const std::string &str) {
    style.FontName = copyString(str);
}

static RenderResult getNext(const RenderResult &res) { return *res.next; }
